...
- (void)viewDidLoad
{
    ...
    welcom=[[WelcomViewController alloc]init];
    //============新浪初始化===========================
    ...
    imageView1.image=[UIImage imageNamed:@"loginback"];
    imageView1.alpha=1;
    ...
    imageView2.image=[UIImage imageNamed:@"registerback"];
    imageView2.alpha=1;
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    if (numberSum==6) {
        ...
        NSString *stringState=[dict objectForKey:@"state"];
        NSString *stringUidSina=[dict objectForKey:@"uuid"];
        if ([stringState isEqualToString:@"weibang"]) {
            infodoneView=[[write_done alloc]initWithFrame:CGRectMake(0, 0, 320, 460*mainhight)];
            infodoneView.backgroundColor=[UIColor grayColor];
            infodoneView.imgView.image=imageSina;
            infodoneView.field1.text=stringNameSina;
            ...
            NSString *content=stringUidSina;
    ...
    if (numberSum==0) {//注册完成之后调用
        ...
        NSString *stringPicUrl=[dict objectForKey:@"avatar_large"];
        NSURL *url=[NSURL URLWithString:stringPicUrl];
        imageSina=[UIImage imageWithData:[NSData dataWithContentsOfURL:url]];//全局变量新浪图片
        NSLog(@"输出照片：%@",imageSina);
        NSString *sexSina=[dict objectForKey:@"gender"];
        NSLog(@"输出性别：%@",sexSina);
        stringNameSina=[dict objectForKey:@"name"];
        NSLog(@"输出用户名：%@",stringNameSina);//全局变量新浪名字
        NSString *stringUuidSina=[dict objectForKey:@"id"];
        NSString *stringLocalSina=[dict objectForKey:@"location"];
        NSString *stringAgeSina=@"0";
        NSString* strSina=@"mac/user/IF00106";
        NSString* strURLSina=globalURL(strSina);
        NSURL* urlBangSina=[NSURL URLWithString:strURLSina];
        NSString *stringUidSelf=@"0000";
        ...
        [rrequest setPostValue:stringUidSelf forKey: @"uuid"];
        [rrequest setPostValue:stringUuidSina forKey: @"suid"];
        [rrequest setPostValue:stringNameSina forKey:@"user_nick"];
        [rrequest setPostValue:stringPicUrl forKey:@"user_pic"];
        [rrequest setPostValue:sexSina forKey:@"user_sex"];
        [rrequest setPostValue:stringLocalSina forKey:@"user_local"];
        [rrequest setPostValue:stringAgeSina forKey:@"user_age"];
        [rrequest setDelegate:self];
        [rrequest startAsynchronous];
        numberSum=6;
    }
...
-(void)doneWon{
    ...
    if((self.user_sex==nil)||(self.user_age==nil)){
        ...
        FriendViewView *friend=[[FriendViewView alloc]initWithFrame:CGRectMake(0, 0, 320, mainscreenhight)];
        [friend.buttonCancel addTarget:self action:@selector(goToMainView) forControlEvents:UIControlEventTouchUpInside];
        [friend.buttonAffirm addTarget:self action:@selector(goToMainView) forControlEvents:UIControlEventTouchUpInside];
...
-(void)affirm{
    ...
    infowriteView=[[write_infor alloc]initWithFrame:CGRectMake(0, 0, 320, mainscreenhight)];
    numberSum=0;
    ...
    if(self.user_pic)
        infowriteView.imgView.image=self.user_pic;
    
    [infowriteView.button1 setSelected:YES];
